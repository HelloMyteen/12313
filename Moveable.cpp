...
<<<<<<< HEAD
	...
	if (moveable&&moveable->initWithFile(filename))
	{
		...
		//    moveable->setEffectRange(cocos2d::Sprite::create("∑???"));
...
<<<<<<< HEAD
	...
	{
		auto target = static_cast<Sprite*>(event->getCurrentTarget());//aò?°μ?μ±?∞￥?√?μ???±í

		cocos2d::Point locationInNode = target->convertToNodeSpace(touch->getLocation());
		cocos2d::Size s = target->getContentSize();
		cocos2d::Rect rect = cocos2d::Rect(0, 0, s.width, s.height);
		if (rect.containsPoint(locationInNode))//≈–??￥?√?μ???∑ò‘???±íμ?∑?????
		{
			if (message.getTargetOne() == nullptr)//target1????±í
			{
				if (false)//欲操控的单位不属于玩家，直接结束该操作
				{
					//auto toast = Toast::create("You just can controll your unit");
					//message._mapscene->addChild(toast);
					return true;
				}
				if (message.getOrder() != "sell")
				{
...
<<<<<<< HEAD
	...
	if (type == TANK_TYPE)
	{
		...
		auto frame = cocos2d::SpriteFrame::create("tankright.png", cocos2d::Rect(0, 0, 99, 99));
	...
	else if (type == DOG_TYPE)
	{
		...
		auto frame = cocos2d::SpriteFrame::create("dogright.png", cocos2d::Rect(0, 0, 99, 99));
		this->setDisplayFrame(frame);
	}
	setPosition(pos);
	scheduleUpdate();
